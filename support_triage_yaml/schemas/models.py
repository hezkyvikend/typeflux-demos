...
class Classification(BaseModel):
    ...
    def _clamp_unjustified_critical(self) -> Classification:
        """Keep critical urgency aligned with the reduced inline-step contract.

        In the YAML demo, ``classify_ticket`` is an inline step with no Python
        hook, so the schema enforces the remaining invariant it can express from
        model-local data alone: a ``critical`` classification must also carry a
        strongly negative sentiment signal.
        """
        if self.urgency is Urgency.CRITICAL and self.sentiment >= -0.3:
