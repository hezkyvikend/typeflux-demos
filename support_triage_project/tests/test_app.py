...
from support_triage_project.demo import sample_ticket
from support_triage_project.main import run_sample_ticket
from support_triage_project.observers import RecordingObserver
from support_triage_project.schemas import (
    Classification,
    DraftReply,
    ReviewPacket,
    RoutingDecision,
...
def test_app_runs_sample_ticket_end_to_end_with_observer() -> None:
    observer = RecordingObserver()
    packet = run_sample_ticket(
    ...
    assert {event.step_name for event in observer.events} == {
        "classify_ticket",
        "route_ticket",
        "draft_response",
        "package_for_review",
...
def test_pipeline_uses_yaml_step_order() -> None:
    assert [step.name for step in SupportTriageWorkflow.steps] == [
        "classify_ticket",
        "route_ticket",
        "draft_response",
        "package_for_review",
    ]
    assert SupportTriageWorkflow.steps[0].has_hook is False
    assert [step.has_hook for step in SupportTriageWorkflow.steps[1:]] == [True, True, True]


def test_classification_model_validator_clamps_critical_without_strong_negative_sentiment() -> None:
    classification = Classification(
        category="billing",
        urgency=Urgency.CRITICAL,
        sentiment=-0.2,
        topics=["Billing", "Invoice", "billing"],
    )

    assert classification.urgency is Urgency.HIGH
    assert classification.topics == ["billing", "invoice"]
